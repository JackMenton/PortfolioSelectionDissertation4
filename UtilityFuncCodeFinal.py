...
lam = 0.2 # temperature parameter for entropy
...
def surface_plot(matrix1, matrix2, x_vec, y_vec, **kwargs):
    ''' Function to create 3d plot - with true surface'''
...
def surface_plot2(matrix1, x_vec, y_vec, **kwargs):
    ''' Function to create 3d plot  - without true surface'''
    (x, y) = np.meshgrid(x_vec, y_vec)
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    surf1 = ax.plot_surface(x, y, matrix1, label = 'Approximated Surface', **kwargs)
...
# ------ Main ------ # 
agent = Agent(alpha = 0.00325, beta = 0.00325, input_dims = [2], gamma = 1, l1_size = 32, l2_size = 32)
...
mean_tw = np.mean(terminal_wealths) # get mean of terminal wealths during testing phase
var_tw = np.var(terminal_wealths) # get variance of terminal wealths during testing phase

mean_actions_init = np.mean(testing_actions_init) # get mean of actions during initial state of testing phase
sd_actions_init = np.std(testing_actions_init) # get variance of actions during initial state of testing phase

mean_actions_final = np.mean(testing_actions_final) # get mean of actions during final state of testing phase
sd_actions_final = np.std(testing_actions_final) # get variance of actions during final state of testing phase
...
# Plots actions taken in initial states during testing phases
plt.figure()
plt.plot(range(int(EPOCHS/10)), testing_actions_init, label = 'episode action')
plt.axhline(y=true_mean(x_0), color='r', linestyle='-', label = 'optimal control')
plt.title('Actions Taken in Initial State During Testing Phase', fontsize = 30)
plt.xlabel('Episodes', fontsize = 30)
plt.ylabel('Action', fontsize = 30)
plt.text(EPOCHS-(EPOCHS/10),2.5, textstr)
plt.legend(fontsize = 30)

# Plot of actions taken in final state
plt.figure()
plt.plot(range(len(testing_actions_final)), testing_actions_final, label = 'episode action')
plt.title('Actions Taken in Final State During Testing Phase', fontsize = 30)
plt.xlabel('Episodes', fontsize = 30)
plt.ylabel('Action', fontsize = 30)
plt.text(EPOCHS-(EPOCHS/10),2.5, textstr)
plt.legend(fontsize = 30)
...
x_points = list(np.linspace(0.0, 1.5, 100))
...
# Surface plot with true values
#(fig, ax, surf1, surf2) = surface_plot(values, true_values, x_points, tmat_points)#, cmap=plt.cm.coolwarm)
    
# surface plot without true values
(fig, ax, surf1) = surface_plot2(values, x_points, tmat_points)#, cmap=plt.cm.coolwarm)
...
ax.legend([fake2Dline], ['Approximated Surface'], fontsize = 20)
ax.text(1.4,0,0.3, textstr, fontsize = 20)
